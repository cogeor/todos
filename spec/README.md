...
| **scripts** | `scripts/` | `serve-phone.mjs`, `smoke.mjs` (2) | `spec/infrastructure/README.md` § "Canonical implementation" (×2), `spec/frontend/README.md` § Selector Contract | Both exist. **Each file is written verbatim from the canonical code block in `spec/infrastructure/README.md`** — do not redesign, copy. The prose around each block explains why the code looks the way it does (pre-flight gates, `shell:true` vs `shell:false`, native React setter for the date input). |
